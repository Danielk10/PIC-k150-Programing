...
Arguments:
    ...
    program_pic(port, pic_type, hex_file_name, fuses = fuses,
                ID = ID)
...
class Chipinfo_Reader(object):
                ...
                if (lhs == 'CHIPname'):
                    ...
                    try:
                        if (lhs in special_handlers):
                            entry[lhs] = special_handlers[lhs](rhs)
                        else:
                            entry[lhs] = rhs
                    except NameError:
    ...
    def get_chip(self, name):
        return Chipinfo_Entry(**self.chip_entries[name])
...
class Protocol_Interface(object):
    ...
    def read_rom(self):
        ...
        response = self.port.read(rom_size)
    ...
    def read_eeprom(self):
        ...
        self._set_programming_voltages_command(True)
        self.port.write(chr(cmd))
        response = self.port.read(eeprom_size)
        self._set_programming_voltages_command(False)
        self._command_end()
        return data_string.data_string(response)
    ...
    def read_calibration(self):
        "Reads calibration value from chip."
        cmd = 14
        self._need_vars()
        ...
        self.port.write(chr(cmd))
        response = self.port.read(2)
        result, = struct.unpack('>H', response)
        self._set_programming_voltages_command(False)
        self._command_end()
        return result
    ...
    def erase_chip(self):
        "Erases all information on the PIC."
        cmd = 15
        self._need_vars()
    ...
    def rom_is_blank(self, high_byte):
        ...
        expected_ff_bytes = (self.vars['rom_size'] // 256) - 1
        self._command_start(cmd)
        ...
        while True:
            ...
            if (response == 'N'):
                ...
                return False
            if (response == '\xff'):
                if (expected_ff_bytes <= 0):
                    raise self.Invalid_Response_Error, "Received wrong number of 0xff bytes in rom_is_blank()"
                expected_ff_bytes -= 1
            else:
...
class prot_interface_wrangler(object):
    def __init__(self,
        ...
        self.port = serial.Serial(port, 19200, timeout = 5)
...
def program_pic(port, pic_type, hex_file_name = '',
    ...
    rom_records = range_filter_records(hex_file.records, 0, 0x4000)
    config_records = range_filter_records(hex_file.records, 0x4000, 0x4010)
    eeprom_records = range_filter_records(hex_file.records, 0x4200, 0xffff)
    ...
    if (response != hex_file_name):
        raise Protocol_Interface.Invalid_Response_Error, 'Echo failed.'

    # Initialize programming variables
    prot_interface.init_programming_vars(**chip_info.get_programming_vars())

    # Write ROM, EEPROM, ID and fuses
    print 'Programming ROM'
    prot_interface.program_rom(rom_data)
    print 'Programming EEPROM'
    prot_interface.program_eeprom(eeprom_data)
    print 'Programming ID and fuses'
    prot_interface.program_id_fuses(ID, fuse_values)
    ...
    verification_result = ((pic_rom_data == rom_data) and
                           (pic_eeprom_data == eeprom_data))
