...
public class ChipinfoReader {
    ...
    public ChipinfoReader(Activity actividad) {
        ...
        if (actividad == null) {
            LogManager.e(LogManager.Categoria.DATOS, "La actividad no puede ser nula");
        ...
        try {
            ...
            LogManager.i(LogManager.Categoria.DATOS, "DatosDePic inicializado correctamente");
            ...
            for (String te : datos.getInformacionPic()) {
                ...
                try {
                    ...
                    if (te.equals("")) {
                        cargarDatos();
                        chipsProcesados++;
                        LogManager.d(LogManager.Categoria.DATOS, "Chip procesado: " + CHIPname + " (" + chipsProcesados + ")");
                    }
                    ...
                    if (numeroRegistro == ChipinfoReader.NUMERO_DE_REGISTROS_DATOS) {
                        cargarDatos();
                        chipsProcesados++;
                        LogManager.d(LogManager.Categoria.DATOS, "Último chip procesado: " + CHIPname + " (" + chipsProcesados + ")");
                    }
                    
                } catch (Exception e) {
                    LogManager.e(LogManager.Categoria.DATOS, "Error al procesar línea " + numeroRegistro + ": " + te + " - " + e.getMessage(), e);
                    // Continuamos con la siguiente línea
                }
            }
            
            LogManager.i(LogManager.Categoria.DATOS, "Procesamiento completado. Total de chips: " + chipEntries.size());
            
        } catch (Exception e) {
            LogManager.e(LogManager.Categoria.DATOS, "Error al inicializar ChipinfoReader: " + e.getMessage(), e);
    ...
    private void procesarLinea(String te, int numeroRegistro) {
        ...
        if (te.length() >= 6 && te.substring(0, 6).equals("CPwarn")) {
            ...
            LogManager.v(LogManager.Categoria.DATOS, "CPwarn: " + CPwarn);
        }

        if (te.length() >= 7 && te.substring(0, 7).equals("CALword")) {
            CALword = te.substring(8);
            LogManager.v(LogManager.Categoria.DATOS, "CALword: " + CALword);
        }

        if (te.length() >= 7 && te.substring(0, 7).equals("BandGap")) {
            BandGap = te.substring(8);
            LogManager.v(LogManager.Categoria.DATOS, "BandGap: " + BandGap);
        }

        if (te.length() >= 8 && te.substring(0, 8).equals("ICSPonly")) {
            ICSPonly = te.substring(9);
            LogManager.v(LogManager.Categoria.DATOS, "ICSPonly: " + ICSPonly);
        }

        if (te.length() >= 6 && te.substring(0, 6).equals("ChipID")) {
            ChipID = te.substring(7);
            LogManager.v(LogManager.Categoria.DATOS, "ChipID: " + ChipID);
        }
    ...
    private void procesarFUSEblank(String fuseblankData) {
        LogManager.v(LogManager.Categoria.DATOS, "Procesando FUSEblank: " + fuseblankData);
        ...
        try {
            ...
            LogManager.v(LogManager.Categoria.DATOS, "FUSEblank procesado: " + datosProcesados.length + " valores");
            
        } catch (Exception e) {
            LogManager.e(LogManager.Categoria.DATOS, "Error al procesar FUSEblank: " + e.getMessage(), e);
    ...
    private void procesarLIST(String listData) {
        LogManager.v(LogManager.Categoria.DATOS, "Procesando LIST: " + listData);
        ...
        try {
            ...
            LogManager.v(LogManager.Categoria.DATOS, "LIST procesado: " + datosFuses.length + " elementos");
            
        } catch (Exception e) {
            LogManager.e(LogManager.Categoria.DATOS, "Error al procesar LIST: " + e.getMessage(), e);
    ...
    private void cargarDatos() {
        LogManager.d(LogManager.Categoria.DATOS, "Cargando datos para chip: " + CHIPname);
        
        try {
            // Validar datos obligatorios
            if (CHIPname == null || CHIPname.trim().isEmpty()) {
                LogManager.e(LogManager.Categoria.DATOS, "CHIPname no puede ser nulo o vacío");
                throw new ChipConfigurationException("CHIPname no puede ser nulo o vacío");
            }
            
            if (ROMsize == null || ROMsize.trim().isEmpty()) {
                LogManager.e(LogManager.Categoria.DATOS, "ROMsize no puede ser nulo o vacío para chip: " + CHIPname);
            ...
            if (CoreType == null || CoreType.trim().isEmpty()) {
                LogManager.e(LogManager.Categoria.DATOS, "CoreType no puede ser nulo o vacío para chip: " + CHIPname);
            ...
            LogManager.d(LogManager.Categoria.DATOS, "Chip cargado exitosamente: " + CHIPname);
            
        } catch (Exception e) {
            LogManager.e(LogManager.Categoria.DATOS, "Error al cargar datos para chip " + CHIPname + ": " + e.getMessage(), e);
    ...
    public ChipPic getChipEntry(String chipName) {
        LogManager.d(LogManager.Categoria.DATOS, "Buscando chip: " + chipName);
        
        if (chipName == null || chipName.trim().isEmpty()) {
            LogManager.w(LogManager.Categoria.DATOS, "Nombre de chip no puede ser nulo o vacío");
            return null;
        }
        ...
        if (chipPic == null) {
            LogManager.w(LogManager.Categoria.DATOS, "Chip no encontrado: " + chipName);
        } else {
            LogManager.d(LogManager.Categoria.DATOS, "Chip encontrado: " + chipName);
        }
    ...
    public ArrayList<String> getModelosPic() {
        LogManager.d(LogManager.Categoria.DATOS, "Obteniendo lista de modelos PIC. Total: " + modelosPic.size());
