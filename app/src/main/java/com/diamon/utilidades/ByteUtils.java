 ...
 * 
 * <p>Esta clase proporciona métodos seguros para manipulación de bytes,
 * incluyendo validación, conversión, verificación de integridad y logging
 * automático de operaciones críticas.</p>
 * 
 * <p>Características principales:</p>
 * <ul>
 *   <li>Validación automática de parámetros de entrada</li>
 *   <li>Logging integrado de operaciones de bytes</li>
 *   <li>Verificación de integridad con checksums</li>
 *   <li>Conversiones seguras entre tipos de datos</li>
 *   <li>Manejo de endianness para comunicación USB</li>
 *   <li>Utilidades para formateo y visualización</li>
 * </ul>
 * 
 * @author Danielk10
 * @version 1.0
...
public final class ByteUtils {
    ...
    public static void validarArray(byte[] datos, int tamanoEsperado, String nombreParametro) {
        ...
        if (tamanoEsperado >= 0 && datos.length != tamanoEsperado) {
            String mensaje = String.format("Parámetro '%s' debe tener %d bytes, pero tiene %d", 
        ...
        if (datos.length > MAX_BUFFER_SIZE) {
            String mensaje = String.format("Parámetro '%s' excede el tamaño máximo permitido (%d bytes)", 
                                         nombreParametro, MAX_BUFFER_SIZE);
            LogManager.e(Categoria.DATA, LOG_PREFIX, mensaje);
            throw new IllegalArgumentException(mensaje);
        }
        
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
    ...
    public static void validarRango(byte[] array, int offset, int longitud) {
        if (offset < 0 || longitud < 0 || offset + longitud > array.length) {
            String mensaje = String.format("Rango inválido: offset=%d, longitud=%d, arraySize=%d", 
    ...
    public static String bytesToHex(byte[] datos) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Convertidos %d bytes a hex: %s", datos.length, 
    ...
    public static byte[] intToBytes(int valor, boolean bigEndian) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Int %d convertido a bytes (%s): %s", valor, 
                                bigEndian ? "BE" : "LE", bytesToHex(resultado)));
        return resultado;
    }
    ...
    public static byte[] shortToBytes(short valor, boolean bigEndian) {
        ...
        byte[] resultado = buffer.array();
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Short %d convertido a bytes (%s): %s", valor, 
                                bigEndian ? "BE" : "LE", bytesToHex(resultado)));
        return resultado;
    }
    ...
    public static int bytesToInt(byte[] datos, boolean bigEndian) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Bytes %s convertidos a int (%s): %d", 
    ...
    public static short bytesToShort(byte[] datos, boolean bigEndian) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Bytes %s convertidos a short (%s): %d", 
                                bytesToHex(datos), bigEndian ? "BE" : "LE", resultado));
        return resultado;
    }
     ...
     */
    public static int copiarBytes(byte[] origen, int offsetOrigen, 
                                 byte[] destino, int offsetDestino, int longitud) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Copiados %d bytes desde pos %d a pos %d", 
    ...
    public static void rellenarArray(byte[] array, byte valor, int offset, int longitud) {
        ...
        LogManager.v(Categoria.DATA, LOG_PREFIX, 
                    String.format("Array rellenado con 0x%02X desde pos %d por %d bytes", 
    ...
    public static boolean verificarChecksum(byte[] datos) {
        ...
        LogManager.d(Categoria.DATA, LOG_PREFIX, 
                    String.format("Verificación checksum: calculado=0x%02X, esperado=0x%02X, válido=%s", 
    ...
    public static boolean validarRespuestaUSB(byte[] respuesta, byte esperado, String comando) 
        ...
        if (respuesta.length == 0) {
            LogManager.e(Categoria.USB, LOG_PREFIX, 
        ...
        if (valida) {
            LogManager.v(Categoria.USB, LOG_PREFIX, 
                        String.format("Respuesta USB válida para '%s': 0x%02X", comando, recibido));
        } else {
            LogManager.w(Categoria.USB, LOG_PREFIX, 
                        String.format("Respuesta USB inválida para '%s': esperado=0x%02X, recibido=0x%02X", 
    ...
    public static byte[] prepararDatosUSB(byte[] datos, String comando) {
        ...
        LogManager.d(Categoria.USB, LOG_PREFIX, 
     ...
     */
    public static String formatearParaVisualizacion(byte[] datos, int bytesPorLinea, boolean mostrarAscii) {
        validarArray(datos, -1, "datos");
    ...
    public static String obtenerEstadisticas(byte[] datos) {
        ...
        return String.format("Tamaño: %d bytes, Min: 0x%02X, Max: 0x%02X, Ceros: %d, 0xFF: %d", 
