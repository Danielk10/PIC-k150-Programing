 ...
 *
 * <p>Esta clase proporciona funcionalidad común para todos los protocolos de comunicación
 * con programadores PIC, incluyendo manejo de conexión USB, operaciones básicas de
 * comunicación y logging estructurado de todas las operaciones.</p>
 *
 * <p>Características principales:</p>
 * <ul>
 *   <li>Logging integrado de todas las operaciones USB</li>
 *   <li>Manejo robusto de excepciones específicas del dominio</li>
 *   <li>Operaciones seguras de bytes con validación automática</li>
 *   <li>Timeouts configurables y manejo de errores</li>
 *   <li>Trazabilidad completa de comandos y respuestas</li>
 * </ul>
 *
 * @author Danielk10
...
public abstract class Protocolo {
    ...
    public Protocolo(Context contexto, UsbSerialPort usbSerialPort) {
        ...
        LogManager.i(Categoria.USB, "ProtocoloInit",
    ...
    private void clearBuffer() throws UsbCommunicationException {
        ...
        try {
            ...
            while ((bytesLeidos = usbSerialPort.read(tmpBuffer, 100)) > 0) {
                ...
                LogManager.v(Categoria.USB, "clearBuffer",
            ...
            if (totalLimpiado > 0) {
                LogManager.d(Categoria.USB, "clearBuffer",
    ...
    protected byte[] readBytes(int count, int timeoutMillis) throws UsbCommunicationException {
        ...
        long inicioOperacion = LogManager.logInicioOperacion(Categoria.USB,
                                                           String.format("readBytes(%d)", count));
        ...
        try {
            ...
            LogManager.v(Categoria.USB, "readBytes",
                        String.format("Iniciando lectura de %d bytes con timeout %d ms", count, timeoutMillis));
            ...
            while (byteBuffer.position() < count &&
                ...
                if (bytesRead > 0) {
                    ...
                    LogManager.v(Categoria.USB, "readBytes",
                                String.format("Leídos %d bytes, total: %d/%d",
                    ...
                    try {
                        ...
                        LogManager.e(Categoria.USB, "readBytes", "Hilo interrumpido durante lectura", e);
            ...
            if (byteBuffer.position() < count) {
                ...
                String mensaje = String.format("Timeout leyendo bytes: esperados=%d, leídos=%d, tiempo=%dms",
            ...
            LogManager.logFinOperacion(Categoria.USB, String.format("readBytes(%d)", count),
                                     inicioOperacion, true);
            
            return resultado;
            
        } catch (IOException e) {
            LogManager.e(Categoria.USB, "readBytes",
                        String.format("Error I/O leyendo %d bytes", count), e);
            LogManager.logFinOperacion(Categoria.USB, String.format("readBytes(%d)", count),
     ...
     */
    private void expectResponse(byte[] expected, int timeoutMillis) throws UsbCommunicationException {
        ByteUtils.validarArray(expected, -1, "expected");
        
        String esperadoHex = ByteUtils.bytesToHex(expected);
        LogManager.v(Categoria.USB, "expectResponse",
        ...
        if (!Arrays.equals(response, expected)) {
            LogManager.w(Categoria.USB, "expectResponse",
                        String.format("Respuesta no coincide - Esperado: %s, Recibido: %s",
                                    esperadoHex, recibidoHex));
            throw UsbCommunicationException.crearRespuestaInesperada(esperadoHex, recibidoHex, "expectResponse");
        }
    ...
    protected boolean leerRespuesta(byte[] response, char esperado, String mensajeError)
        ...
        try {
            LogManager.v(Categoria.USB, "leerRespuesta",
            ...
            if (coincide) {
                ...
                LogManager.w(Categoria.USB, "leerRespuesta",
                           String.format("%s - Esperado: '%c' (0x%02X), Recibido: 0x%02X",
    ...
    protected void enviarComando(String comando) throws UsbCommunicationException {
        ...
        try {
            LogManager.i(Categoria.USB, "enviarComando",
            ...
            if (ack[0] != 'P') {
                String mensaje = String.format("Acknowledgment inválido: esperado 'P', recibido 0x%02X", ack[0]);
                LogManager.e(Categoria.USB, "enviarComando", mensaje);
                throw UsbCommunicationException.crearRespuestaInesperada("P",
                                                                        String.format("0x%02X", ack[0]),
                                                                        "enviarComando");
            }
            ...
            if (data[0] != 0) {
                ...
                LogManager.v(Categoria.USB, "enviarComando",
                           String.format("Enviado comando final: %s", ByteUtils.bytesToHex(comandoFinal)));
            } else {
                LogManager.v(Categoria.USB, "enviarComando", "Comando 0 - no se envía data adicional");
            }

            LogManager.logFinOperacion(Categoria.USB, String.format("enviarComando(%s)", comando),
                                     inicioOperacion, true);
            LogManager.d(Categoria.USB, "enviarComando", String.format("Comando '%s' enviado exitosamente", comando));
            
        } catch (IOException e) {
            LogManager.e(Categoria.USB, "enviarComando",
                        String.format("Error I/O enviando comando '%s'", comando), e);
            LogManager.logFinOperacion(Categoria.USB, String.format("enviarComando(%s)", comando),
                                     inicioOperacion, false);
    ...
    public boolean iniciarProtocolo() {
        ...
        try {
            LogManager.i(Categoria.USB, "iniciarProtocolo",
                        String.format("Iniciando protocolo %s", nombreProtocolo));
            
            // Preparar comando de inicialización
            byte[] data = ByteUtils.prepararDatosUSB(comando.getBytes(StandardCharsets.US_ASCII), "inicializacion");
            ...
            if (respuesta.length > 1) {
                LogManager.w(Categoria.USB, "iniciarProtocolo", "Datos adicionales en buffer, limpiando...");
                clearBuffer();
            ...
            if (exitoso) {
                ...
                LogManager.w(Categoria.USB, "iniciarProtocolo",
                           String.format("Protocolo %s falló inicialización - respuesta: 0x%02X",
            ...
            LogManager.e(Categoria.USB, "iniciarProtocolo", "Error de formato en inicialización", e);
    ...
    public boolean esperarInicioDeNuevoComando() {
        ...
        long inicioOperacion = LogManager.logInicioOperacion(Categoria.USB, "esperarInicioDeNuevoComando");
        ...
        try {
            ...
            LogManager.d(Categoria.USB, "esperarInicio", "Byte de sincronización enviado exitosamente");
            LogManager.logFinOperacion(Categoria.USB, "esperarInicioDeNuevoComando", inicioOperacion, true);
            return true;

        } catch (IOException e) {
            LogManager.e(Categoria.USB, "esperarInicio", "Error I/O enviando sincronización", e);
            LogManager.logFinOperacion(Categoria.USB, "esperarInicioDeNuevoComando", inicioOperacion, false);
            return false;
        }
    }
    ...
    protected void logEstadoProtocolo() {
        ...
        LogManager.i(Categoria.USB, "EstadoProtocolo",
                    String.format("Protocolo: %s, Puerto USB: %s",
    ...
    protected long logInicioOperacionChip(String operacion, ChipPic chipPIC) throws ChipConfigurationException {
        ...
        LogManager.i(Categoria.CHIP, operacion,
                    String.format("Iniciando %s - %s", operacion, infoChip));
     ...
     */
    protected void logFinOperacionChip(String operacion, long inicioTimestamp,
                                     boolean exitosa, int bytesProcessados) {
        long duracion = System.currentTimeMillis() - inicioTimestamp;
        String resultado = exitosa ? "ÉXITO" : "FALLO";
        String info = bytesProcessados >= 0 ?
            String.format(" (%d bytes procesados)", bytesProcessados) : "";
        
        LogManager.i(Categoria.CHIP, operacion,
                    String.format("%s completada - %s en %dms%s",
                                operacion, resultado, duracion, info));
    }
    ...
    protected void escribirDatosUSB(byte[] datos, int timeoutMillis, String descripcion)
        ...
        try {
            LogManager.v(Categoria.USB, "escribirDatos",
            ...
            LogManager.e(Categoria.USB, "escribirDatos",
                        String.format("Error escribiendo %s", descripcion), e);
